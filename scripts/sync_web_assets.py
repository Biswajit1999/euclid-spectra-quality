...
def sync(results_dir: Path, figures_dir: Path, manifest_path: Path, catalogue_csv_path: Path, web_public_dir: Path) -> None:
    ...
    if catalogue_csv_path.is_file():
        shutil.copy2(catalogue_csv_path, web_public_dir / "source_catalog.csv")
        copied.append(str(catalogue_csv_path))
...
def main() -> None:
    ...
    sync(args.results_dir, args.figures_dir, args.manifest, args.catalogue_csv, args.web_public_dir)
